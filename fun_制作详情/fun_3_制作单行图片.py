...
def fun_制作单行图片(
    image_list: list[ImageModel], item: MakeProductPageModel
) -> Image.Image:
    col_width = (
        item.product_page_width - ((len(image_list) + 1) * item.spacing)
    ) / sum([k.ratio for k in image_list])
    ...
    for image in image_list:
        ...
        if height > 2000:
            width = int(
                (
                    item.product_page_width
                    - ((len(image_list) + 1) * item.spacing)
                )
                / len(image_list)
            )
            height = 2000
    ...
    im = fun_图片居中粘贴(
        im, item.product_page_width, im.height, (255, 255, 255, 255)
    )
...
def fun_制作单个图片(
    ...
    if item.contains_image_information:
