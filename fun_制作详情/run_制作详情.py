...
from .type import MakeXQModel


def run_制作文件夹详情(item: MakeXQModel):
    """
    ...
    if len(item.img_list) > 0:
        image_list = fun_构建指定图片(img_list=item.img_list)
    else:
        image_list = fun_构建所有图片(
            path=item.image_dir_path, use_iamge=item.use_image, sort=item.sort
        )
